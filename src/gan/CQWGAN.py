...
def train_imported_gan(train_dataloader, classes: list, out_folder: str, qasm_file_path: str,
    ...
    classes_str = ''.join(str(cls) for cls in classes)
    ...
    save_generated_images(generator=generator, latent_vector=initial_latent_vector, out_dir=out_dir,
                          image_name="0")
    # Save a sample of real images
    save_real_images(real_images=real_images, out_dir=out_dir, image_name="real_images_sample")
    ...
    for epoch in range(n_epochs):
        ...
        for i, (real_images, _) in enumerate(train_dataloader):  # Iterate over batches in the data loader.
            ...
            if i % n_critic == 0: # Train the generator every n_critic steps
                ...
                print(
                    f"[Epoch {epoch}/{n_epochs}] [Batch {i}/{len(train_dataloader)}] [D loss: {d_loss.item()}] [G loss: {g_loss.item()}] [Wasserstein Distance: {wasserstein_distance.item()}]")
                # TODO: do i need this?
